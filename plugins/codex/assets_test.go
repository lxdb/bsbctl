...
func TestCodexPackageDeclaresOnlyTheVerifiedStaticMark(t *testing.T) {
	t.Parallel()
	...
	if got := hex.EncodeToString(digest[:]); got != "965cce3d5fe4ea1f11a507f2ea5e1474b11894239b88e48268a223be780f708c" {
		t.Fatalf("static mark sha256 = %s", got)
	}
	if declaration.SHA256 != hex.EncodeToString(digest[:]) || declaration.Size != int64(len(content)) {
		t.Fatalf("asset integrity declaration = %#v", declaration)
	}
