...
When a daemon channel policy sets `activation_input: "start"` or `"start_or_encoder"` together with `activation_action`, the activating input is delivered once through this input path after the exact session is promoted. `start` forwards START only. `start_or_encoder` also lets an encoder event activate the exact observation. A foreground session receives its input directly. An unset policy preserves activation without forwarding input.
