...
func TestDecodePlanRequiresIndependentStableComponentMetadata(t *testing.T) {
	...
	plan, err := DecodePlan([]byte(`{"schema_version":1,"components":[` +
		...
		`{"id":"dev.bsbctl.slack","kind":"plugin","version":"0.3.0","tag":"plugin/slack/v0.3.0","package":"./cmd/bsbctl-plugin-slack","binary":"bsbctl-plugin-slack"}]}`))

	if err != nil {
		t.Fatalf("DecodePlan: %v", err)
	}
	if len(plan.Components) != 7 || plan.Components[0].Tag != "v0.1.0" || plan.Components[1].Tag != "plugin/calendar/v0.1.0" || plan.Components[2].Tag != "plugin/codex/v0.1.0" || plan.Components[3].Version != "0.2.0" || plan.Components[4].Tag != "plugin/github-notifications/v0.1.0" || plan.Components[5].Version != "1.0.0" || plan.Components[6].Version != "0.3.0" {
