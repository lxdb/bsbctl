...
func releaseRoot(t *testing.T, legalBlocker, keyProvisioned, localReplace bool) string {
	...
	for _, packagePath := range []string{"cmd/bsbctl", "cmd/bsbctl-plugin-calendar", "cmd/bsbctl-plugin-codex", "cmd/bsbctl-plugin-codex-quota", "cmd/bsbctl-plugin-github-notifications", "cmd/bsbctl-plugin-mac-resources"} {
