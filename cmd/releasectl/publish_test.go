...
func TestBuildReleaseSpecsBindsExactInspectedAssetsAndVerifiedCatalog(t *testing.T) {
	...
	if len(specs) != 5 || specs[0].Tag != "plugin/calendar/v0.5.0" || specs[1].Tag != "plugin/codex-quota/v0.2.0" || specs[2].Tag != "plugin/codex/v0.4.0" || specs[3].Tag != "plugin/mac-resources/v0.3.0" || specs[4].Tag != "v0.1.0" {
		t.Fatalf("release specs = %#v", specs)
	}
	if len(specs[0].Assets) != 2 || len(specs[1].Assets) != 2 || len(specs[2].Assets) != 2 || len(specs[3].Assets) != 2 || len(specs[4].Assets) != 7 {
		t.Fatalf("release asset counts = %d, %d, %d, %d, %d", len(specs[0].Assets), len(specs[1].Assets), len(specs[2].Assets), len(specs[3].Assets), len(specs[4].Assets))
	}
	coreAssetNames := make([]string, 0, len(specs[4].Assets))
	for _, asset := range specs[4].Assets {
