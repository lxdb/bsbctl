# Security policy

## Supported versions

Security reports are evaluated against the latest published release. This project does not currently promise security fixes for older releases.

## Report a vulnerability

Use [GitHub Private Vulnerability Reporting](https://github.com/lxdb/bsbctl/security/advisories/new) when the form is available. Do not disclose an unpatched vulnerability in a public issue.

If the form is unavailable, the repository has no documented confidential reporting channel. Do not include vulnerability details in a public channel. Maintainers must enable and verify the form before public distribution.

Include the affected version or commit, impact, reproduction steps, and any suggested mitigation. Remove active credentials, device identifiers, calendar details, Codex session content, and unrelated personal data.

This project does not promise an acknowledgement or resolution time until maintainers publish and staff a response process.
